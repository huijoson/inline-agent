...
| **訂位人姓名** | `王大明` | 將透過 React 內部 Setter 穿透寫入。 |
| **稱謂** | `先生 / 小姐` | 自動點選 Radix UI 單選項目。 |
...
1. 自動點選目標日期並在 150ms 內連續抓取第一志願時段。
2. 時段鎖定後，立即自動點擊下方「**完成預訂**」按鈕。
3. 若跳出店家「規則與注意事項」，自動勾選並秒按「**我已閱讀並同意**」。
4. 進入聯絡資訊頁面，自動填妥姓名、電話、Email 與稱謂。
5. 自動點擊「**確認訂位**」送出預訂！
...
### Q3：為什麼之前會出現「訂位人姓名不可留空」紅字錯誤？
* inline 是採用 React 架構開發的受控表單。若直接賦值 `input.value = "xxx"` 會被 React 內部狀態駁回重設。最新版助手已全面採用 `HTMLInputElement.prototype` 原生 Setter 寫入，保證 100% 成功注入。
